...
void main()
{
    ...
    float resultAlpha=textureColorOri.a*(1.0/(textureColorOri.a+0.001));
    // step1 curve
    redCurveValue = texture2D(curve, vec2(textureColor.r, 0.0)).r;
    greenCurveValue = texture2D(curve, vec2(textureColor.g, 0.0)).g;
    blueCurveValue = texture2D(curve, vec2(textureColor.b, 0.0)).b;
    // step2 level
    redCurveValue = texture2D(curve, vec2(redCurveValue, 0.0)).a*resultAlpha;
    greenCurveValue = texture2D(curve, vec2(greenCurveValue, 0.0)).a*resultAlpha;
    blueCurveValue = texture2D(curve, vec2(blueCurveValue, 0.0)).a*resultAlpha;
    ...
    textureColor = vec4(redCurveValue, greenCurveValue, blueCurveValue, 0.0);
    textureColor = (textureColorOri - textureColor) * 0.549 + textureColor;

    vec4 resultColor = vec4(textureColor.r, textureColor.g, textureColor.b, textureColorOri.a);
    gl_FragColor =mix(textureColorOri, resultColor, strength);
}
