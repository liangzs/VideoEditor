...
void main(){
    ...
    vec4 orignalColor = texture2D(inputImageTexture, textureCoordinate);
    float resultAlpha=orignalColor.a*(1.0/(orignalColor.a+0.00001));
    ...
    gl_FragColor.rgb = mix(orignalColor.rgb, gl_FragColor.rgb, strength);
     gl_FragColor = vec4(gl_FragColor.rgb, 1.0*resultAlpha);
}