...
vec4 gaussianBlur(sampler2D sampler)
{
    ...
    step.x = 1.49583 * texelWidthOffset;
    step.y = 1.49583 * texelHeightOffset;
    ...
    step.x = 2.4719250988753685 * texelWidthOffset;
    step.y = 2.4719250988753685 * texelHeightOffset;
    ...
    step.x = 5.49583 * texelWidthOffset;
    step.y = 5.49583 * texelHeightOffset;
    ...
    step.x = 5.300352223621558 * texelWidthOffset;
    step.y = 5.300352223621558 * texelHeightOffset;
    color += texture2D(sampler, textureCoordinate+step) * 0.0391;
    color += texture2D(sampler, textureCoordinate-step) * 0.0391;
    color += texture2D(sampler, textureCoordinate+vec2(-step.x, step.y)) * 0.0391;
    color += texture2D(sampler, textureCoordinate+vec2(step.x, -step.y)) * 0.0391;

    step.x = 9.49583 * texelWidthOffset;
    step.y = 9.49583 * texelHeightOffset;
    ...
    step.x = 8.128779348367749 * texelWidthOffset;
    step.y = 8.128779348367749 * texelHeightOffset;
...
void main()
{
    ...
    gl_FragColor = mix(c1, c3, strength)*resultAlpha;
}