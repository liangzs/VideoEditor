...
void main()                    		
{
    ...
    if(v_Paticle==3.0){
        gl_FragColor=texture2D(u_Texture4, gl_PointCoord);
        return;
    }else if(v_Paticle==4.0){
        gl_FragColor=texture2D(u_Texture5, gl_PointCoord);
        return;
    }else if(v_Paticle==5.0){
        gl_FragColor=texture2D(u_Texture6,gl_PointCoord);
        return;
    }else if(v_Paticle==6.0){
        gl_FragColor=texture2D(u_Texture2,gl_PointCoord);
        return;
    }
