 ...
 */
public class ShadeMultiSquareTransitionFilter extends TransitionFilter {
    ...
    private static final String FRAGMENT = FRAGMENT_HEAD +
            ...
            "    float a = step(0.0, xa) * step(0.0, ya);\n" +
    ...
    public ShadeMultiSquareTransitionFilter(TransitionType transitionType) {
        super(transitionType, NO_FILTER_VERTEX_SHADER, FRAGMENT);
        LogUtils.i("FRAGMENT", FRAGMENT);
        ratio = 1.0f;
        switch (transitionType) {
            case SHADE_MULTI_SQUARE:
                isReverse = false;
                break;
            case SHADE_MULTI_SQUARE_RE:
                isReverse = true;
                break;
