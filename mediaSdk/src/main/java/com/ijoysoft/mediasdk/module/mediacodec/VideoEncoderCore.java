...
public class VideoEncoderCore {
    ...
    public VideoEncoderCore(int width, int height, int bitRate, File outputFile)
        ...
        if (VERBOSE) LogUtils.d(TAG, "format: " + format);
    ...
    public void release() {
        if (VERBOSE) LogUtils.d(TAG, "releasing encoder objects");
        if (mEncoder != null) {
            mEncoder.stop();
            mEncoder.release();
            mEncoder = null;
    ...
    public void drainEncoder(boolean endOfStream) {
        ...
        if (VERBOSE) LogUtils.d(TAG, "drainEncoder(" + endOfStream + ")");

        if (endOfStream) {
            if (VERBOSE) LogUtils.d(TAG, "sending EOS to encoder");
        ...
        while (true) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (!endOfStream) {
                    ...
                    if (VERBOSE) LogUtils.d(TAG, "no output available, spinning to await EOS");
                }
                ...
                if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    if (!endOfStream) {
                        ...
                        if (VERBOSE) LogUtils.d(TAG, "end of stream reached");
                    }
                    break;      // out of while
                }
            }
