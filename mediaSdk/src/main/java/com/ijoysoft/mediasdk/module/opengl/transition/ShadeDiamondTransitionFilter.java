 ...
 * 菱形形状
 */
public class ShadeDiamondTransitionFilter extends TransitionFilter {
    ...
    private static final String FRAGMENT = FRAGMENT_HEAD +
            "uniform int reverse;\n" +
            "float diamond(float w) {\n" +
            "   float y0 = w * (abs(textureCoordinate.x - 0.5) + abs(textureCoordinate.y - 0.5)) + 1.0;\n" +
            "   float y1 = w * (abs(textureCoordinate.x - 0.5) + abs(textureCoordinate.y - 0.5)) -w * sqrt(2.0) / 2.0;\n" +
            "   float alpha = clamp(y0 + progress * (-1.0 - w), 0.0, 1.0);" +
            "   return alpha;\n" +
            "}\n" +
            "float diamondRe(float w) {\n" +
            "   float y0 = w * (abs(textureCoordinate.x - 0.5) + abs(textureCoordinate.y - 0.5)) + 1.0;\n" +
            "   float y1 = w * (abs(textureCoordinate.x - 0.5) + abs(textureCoordinate.y - 0.5)) -w * sqrt(2.0) / 2.0;\n" +
            "   float alpha = clamp(y1 + progress * (1.0 + w), 0.0, 1.0);" +
            "   return alpha;\n" +
            "}\n" +
            "void main() {\n" +
            "float w=20.0;" +
            ...
            " if(reverse==0){ " +
            "gl_FragColor = mix(texColor1, texColor2, diamond(w));" +
            "return;" +
            "} " +
            "gl_FragColor = mix(texColor2,texColor1 , diamondRe(w));" +
            "}";


    public ShadeDiamondTransitionFilter(TransitionType transitionType) {
        super(transitionType, NO_FILTER_VERTEX_SHADER, FRAGMENT);
        switch (transitionType) {
            case SHADE_DIAMOND:
                isReverse = false;
                break;
            case SHADE_DIAMOND_RE:
                isReverse = true;
                break;
        }
    }
