 ...
 * 这里在悬停缩放时，添加滤镜
 */
public class TravelThreeThemeOne extends BaseBlurThemeExample {
    private BaseThemeExample widgetBorder;
    private BaseThemeExample widgetOne;
    private BaseThemeExample widgetTwo;

    public TravelThreeThemeOne(int totalTime, boolean isNoZaxis, int width, int height) {
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float scale = width < height ? 1.5f : (width == height ? 2f :3f);
        float centerX = 0;
