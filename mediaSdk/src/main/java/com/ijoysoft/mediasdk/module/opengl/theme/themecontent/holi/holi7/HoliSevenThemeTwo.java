 ...
 * stay 放大-缩小-放大
 */
public class HoliSevenThemeTwo extends BaseBlurThemeExample {
    private BaseThemeExample widgetOne;
    private BaseThemeExample widgetTwo;


    public HoliSevenThemeTwo(int totalTime, boolean isNoZaxis, int width, int height) {
        ...
        widgetTwo.setEnterAnimation(new AnimationBuilder(DEFAULT_ENTER_TIME).setIsNoZaxis(true).setZView(0).
                setCoordinate(0, 2f, 0f, 0f).build());
    }
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float scale = width < height ? 1.1f : (width == height ? 1.2f : 1.2f);
        float centerX = 0;
