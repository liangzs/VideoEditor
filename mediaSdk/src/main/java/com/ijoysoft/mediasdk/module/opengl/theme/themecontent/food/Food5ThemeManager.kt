...
class Food5ThemeManager : ThemeManagerWithwidget() {
    ...
    val widgetDurations: Array<Array<Triple<Int, Int, Int>>> = arrayOf(
        ...
        arrayOf(Triple(ENTER, STAY, OUT)),
        arrayOf(Triple(ENTER, STAY, OUT), Triple(ENTER, STAY, OUT)),
        //
    )
    ...
    val widgetActions: List<Array<Array<BaseEvaluate?>?>?> = listOf(
        ...
        arrayOf(
            arrayOf(
                ThemeWidgetHelper.springMove(ENTER, AnimateInfo.ORIENTATION.BOTTOM),
                null,
                null,
            )),
        //3
        arrayOf(
            arrayOf(
                ThemeWidgetHelper.normalMove(ENTER, AnimateInfo.ORIENTATION.LEFT),
                null,
                null,
            ),
            arrayOf(
                ThemeWidgetHelper.normalMove(ENTER, AnimateInfo.ORIENTATION.RIGHT),
                null,
                null,
            ),
        ),
    ...
    override fun createWidgetLocation(index: Int, widgetIndex: Int, widget: BaseThemeExample, bitW: Int, bitH: Int) {
        when (index) {
            ...
            1 -> {
                widget.adjustScaling(width, height, bitW, bitH, 0f, 0.75f, 2f)
            }
            2 -> {
                when (widgetIndex) {
                    0 -> {
                        widget.adjustScaling(width, height, bitW, bitH, -0.4f, 0.8f, (width < height).ternary(4f, 4.5f))
                    }
                    1 -> {
                        widget.adjustScaling(width, height, bitW, bitH, 0.4f, 0.8f, (width < height).ternary(4f, 4.5f))
                    }
    ...
    override fun themeTransition()
            : TransitionType {
        return TransitionType.PAG_BOOM1
    }
