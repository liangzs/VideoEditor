 ...
 * 所以这里暂时不回收让系统自己回收
 */
public class VTFourThemeManager extends BaseThemeManager {
    public static float CUSTOM_ZVIEW = -2.5f;
    public static float Z_AXIS = -0.2f;
...
//        actionRender = new BirthDayTwoThemeOne((int) mediaItem.getDuration(),true);
        switch (index % 5) {
            case 0:
                actionRender = new VTFourThemeOne((int) mediaItem.getDuration());
                break;
            case 1:
                actionRender = new VTFourThemeTwo((int) mediaItem.getDuration());
                break;
            case 2:
                actionRender = new VTFourThemeThree((int) mediaItem.getDuration());
                break;
            case 3:
                actionRender = new VTFourThemeFour((int) mediaItem.getDuration());
                break;
            case 4:
                actionRender = new VTFourThemeFive((int) mediaItem.getDuration());
                break;
        }
    ...
    public void ratioChange() {
        bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/vt_four_theme_bg"));
...
//        index = 3;
        switch (index % 5) {
            case 0:
                temp = new VTFourThemeOne((int) mediaItem.getDuration());
                break;
            case 1:
                temp = new VTFourThemeTwo((int) mediaItem.getDuration());
                break;
            case 2:
                temp = new VTFourThemeThree((int) mediaItem.getDuration());
                break;
            case 3:
                temp = new VTFourThemeFour((int) mediaItem.getDuration());
                break;
            case 4:
                temp = new VTFourThemeFive((int) mediaItem.getDuration());
                break;
        }
