...
/**
 * 通用Action类
 * 此类与 {@link BaseBlurThemeTemplate} 除了父类以外代码一致，若有改动务必共同修改！！！！！！！！！
 * @author hayring
 * fork from: {@link com.ijoysoft.mediasdk.module.opengl.theme.themecontent.birthday.birthday1.BirthDayActionOne}
 */
public abstract class BaseThemeTemplate extends BaseThemeExample {
    ...
    public BaseThemeTemplate(int totalTime, int width, int height) {
    ...
    public BaseThemeTemplate(int totalTime, int width, int height, boolean isNoZaxis) {
    ...
    public BaseThemeTemplate(int totalTime, int width, int height, boolean enterOutDiy, boolean isNoZaxis) {
        super(totalTime, DEFAULT_ENTER_TIME, DEFAULT_STAY_TIME, DEFAULT_OUT_TIME, isNoZaxis);
        this.width = width;
        this.height = height;
        stayAction = initStayAction();
    ...
    public BaseThemeTemplate(int duration, int enterTime, int stayTime, int outTime, boolean isNoZaxis) {
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        //增加判断防止未适配的主题报错
        int scaleIndex = width < height ? 2 : (widgetsMeta != null && widgetsMeta.length > 0 &&  width == height && widgetsMeta[0].length > 4)? 4 : 3;
            ...
            if (widgetsMeta[i][0] == Float.MAX_VALUE || widgetsMeta[i][0] == Float.MIN_VALUE) {
                ...
                if (widgetsMeta[i][0] == Float.MAX_VALUE) {
                    ...
                    if (widgetsMeta[i][1] == Float.MAX_VALUE) {
                        //往右下边贴
                        orientation = AnimateInfo.ORIENTATION.RIGHT_BOTTOM;
                    } else {
                        //往右上边贴
                        orientation = AnimateInfo.ORIENTATION.RIGHT_TOP;
                    }
                } else if (widgetsMeta[i][0] == Float.MIN_VALUE) {
                    //往左边贴
                    if (widgetsMeta[i][1] == Float.MAX_VALUE) {
                        //往左下边贴
                        orientation = AnimateInfo.ORIENTATION.LEFT_BOTTOM;
                    } else {
                        //往左上边贴
                        orientation = AnimateInfo.ORIENTATION.LEFT_TOP;
                    }
                } else {
                    throw new IllegalStateException("When x is max, y must be max too!");
                }
                widgets[i].adjustScaling(width, height, mimaps.get(i).getWidth(), mimaps.get(i).getHeight(), orientation, 0f, widgetsMeta[i][scaleIndex]);
...
//        }
     ...
     * @param width  屏幕宽度
     * @param height 屏幕高度
     * @see BaseBlurThemeExample#adjustImageScalingStretch(int, int)
     */
    @Override
...
//        y = y == Float.MAX_VALUE ? 1 : y == Float.MIN_VALUE ? -1 : y;
        ...
        return buildNewScaleInOutTemplateAnimate(totalTime, x, y);
    }
    ...
    public static BaseThemeExample buildNewScaleInOutTemplateAnimate(int totalTime, float x, float y) {
        ...
        //变大
        animationBuilder.setScale(0.0001f, 1f);
        ...
        animationBuilder.setZView(0f);
        //变小
        animationBuilder.setScale(1f, 0.0001f);
