 ...
 * 态度2
 */
public class Attitude2Action3 extends BaseThemeTemplate {


    public Attitude2Action3(int totalTime, int width, int height) {
        super(totalTime, width, height);
    }

    @Override
    protected BaseEvaluate initStayAction() {
        StayAnimation res = new StayAnimation(DEFAULT_STAY_TIME, AnimateInfo.STAY.SPRING_Y, BD_ONE_ZVIEW);
    ...
    protected float[][] getWidgetsMeta() {
        if (widgetsMeta == null) {
            widgetsMeta = new float[][]{
                    //表情
                    {0f, 0.55f, 2.6f, 3f, 2.6f},
            };
        }
        if (width < height) {
            widgetsMeta[0][1] = 0.75f;
        } else {
            widgetsMeta[0][1] = 0.55f;
        }
        return widgetsMeta;
    }
