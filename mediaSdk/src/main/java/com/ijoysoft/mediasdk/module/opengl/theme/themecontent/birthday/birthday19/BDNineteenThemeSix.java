 ...
 * one和two承载前两文件展示移动
 */
public class BDNineteenThemeSix extends BaseThemeExample {
    private BaseThemeExample widgetOne;
    private BaseThemeExample widgetTwo;
    private int width, height;

    public BDNineteenThemeSix(int totalTime, int width, int height) {
        super(totalTime, DEFAULT_ENTER_TIME, 1800, DEFAULT_ENTER_TIME);
        enterAnimation = new AnimationBuilder(DEFAULT_ENTER_TIME).setCoordinate(-BDNineteenThemeManager.Z_VIEW, BDNineteenThemeManager.Z_VIEW, 0, 0).
                setConors(AnimateInfo.ROTATION.Y_ANXIS, 0, 360).setZView(BDNineteenThemeManager.Z_VIEW).setWidthHeight(width, height).build();
        outAnimation = new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, 0f, BDNineteenThemeManager.Z_VIEW).setZView(BDNineteenThemeManager.Z_VIEW).build();
    ...
    public void setPreTeture(List<Integer> listTexture, List<Integer> listConors, List<float[]> listPos, List<GPUImageFilter> listAfilter) {
        ...
        widgetOne.setOutAnimation(new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, 0f, -5.5f)
        ...
        widgetTwo.setEnterAnimation(new AnimationBuilder(3400).setWidthHeight(width, height).setZView(BDNineteenThemeManager.Z_VIEW).
                setConors(AnimateInfo.ROTATION.Z_ANXIS, conor, conor).build());
        widgetTwo.setOutAnimation(new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, 0f, BDNineteenThemeManager.Z_VIEW)
                .setWidthHeight(width, height).setConors(AnimateInfo.ROTATION.Z_ANXIS, conor, conor).setZView(BDNineteenThemeManager.Z_VIEW).build());
    }
