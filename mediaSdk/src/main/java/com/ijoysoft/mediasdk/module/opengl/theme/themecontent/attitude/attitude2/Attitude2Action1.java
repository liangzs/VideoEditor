 ...
 * 态度2
 */
public class Attitude2Action1 extends BaseThemeTemplate {

    public Attitude2Action1(int totalTime, int width, int height) {
        super(totalTime, width, height);
    }

    @Override
    protected BaseEvaluate initStayAction() {
        StayAnimation res = new StayAnimation(DEFAULT_STAY_TIME, AnimateInfo.STAY.ROTATE_Z, width, height);
    ...
    protected BaseEvaluate initEnterAnimation() {
        return new AnimationBuilder(DEFAULT_ENTER_TIME).setCoordinate(2f, 0f, 0, 0).
                setConors(AnimateInfo.ROTATION.Y_ANXIS_RE, 0, 360).setZView(BD_ONE_ZVIEW).build();
    }

    @Override
    protected BaseEvaluate initOutAnimation() {
        return new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, -2f, 2f).
