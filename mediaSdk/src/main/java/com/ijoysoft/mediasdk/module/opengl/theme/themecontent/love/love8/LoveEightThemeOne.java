 ...
 * @ description
 */
public class LoveEightThemeOne extends BaseBlurThemeExample {
    private BaseThemeExample widgetBorder;
    private BaseThemeExample widgetOne;
    private BaseThemeExample widgetTwo;

    public LoveEightThemeOne(int totalTime, boolean isNoZaxis, int width, int height) {
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float scaleX = width == height ? 2.1f : (width < height ? 2 : 4);
        float scaleY = width == height ? 2.1f : (width < height ? 2 : 4);
        float centerX = width == height ? 0f : (width < height ? 0f : 0f);
        float centerY = width == height ? 0.75f : (width < height ? 0.85f : 0.8f);
        ...
        scaleX = width == height ? 2.3f : (width < height ? 2.2f : 4.5f);
        scaleY = width == height ? 2.3f : (width < height ? 2.2f : 4.5f);
        centerX = width == height ? 0f : (width < height ? 0f : 0f);
        centerY = width == height ? 0.77f : (width < height ? 0.86f : 0.81f);
