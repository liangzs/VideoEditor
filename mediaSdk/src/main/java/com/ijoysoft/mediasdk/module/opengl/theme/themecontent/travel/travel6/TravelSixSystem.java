...
public class TravelSixSystem extends BaseParticleSystem {
    ...
    public TravelSixSystem(int maxParticleCount) {
    ...
    private void updateParticle(int index) {
        ...
        if (origin - current < 0.3) {
            current = particles[currentOffset + 2] + 6;
    ...
    private void addParticleImpl(int index, float particleStartTime, boolean isInit) {
        ...
        temp = (2 + random.nextInt(3)) * ConstantMediaSize.getMinWH() / 35;//(20,25,30)
        if (isInit) {
