 ...
 * 这里在悬停缩放时，添加滤镜
 */
public class HDFourteenThemeOne extends BaseBlurThemeExample {
    ...
    public HDFourteenThemeOne(int totalTime, boolean isNoZaxis, int width, int height) {
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float scaleX = width < height ? 4f : (width == height ? 5f : 4f);
        float centerX = width < height ? 0.7f : (width == height ? 0.7f : 0.75f);
        float centerY = width < height ? 0.7f : (width == height ? 0.6f : 0.7f);
