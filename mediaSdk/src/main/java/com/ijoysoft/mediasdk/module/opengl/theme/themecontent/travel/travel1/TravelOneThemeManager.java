 ...
 * 所以这里暂时不回收让系统自己回收
 */
public class TravelOneThemeManager extends BaseThemeManager {
    /**
     * 创建缓存对象，离屏buffer
     */
...
//        index = 4;
        switch (index % 5) {
            case 0:
                actionRender = new TravelOneThemeOne((int) mediaItem.getDuration(), width, height);
                break;
            case 1:
                actionRender = new TravelOneThemeTwo((int) mediaItem.getDuration(), width, height);
                break;
            case 2:
                actionRender = new TravelOneThemeThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                actionRender = new TravelOneThemeFour((int) mediaItem.getDuration(), width, height);
                break;
            case 4:
                actionRender = new TravelOneThemeFive((int) mediaItem.getDuration(), width, height);
                break;
        }
    ...
    public void ratioChange() {
        switch (ConstantMediaSize.ratioType) {
            case _9_16:
            case _3_4:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/travel_one_theme_bg916" + ConstantMediaSize.SUFFIX));
                break;
            case _1_1:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/travel_one_theme_bg11" + ConstantMediaSize.SUFFIX));
                break;
            case _16_9:
            case _4_3:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/travel_one_theme_bg169" + ConstantMediaSize.SUFFIX));
                break;
            default:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/travel_one_theme_bg916" + ConstantMediaSize.SUFFIX));
                break;
...
//        index = 4;
        switch (index % 5) {
            case 0:
                temp = new TravelOneThemeOne((int) mediaItem.getDuration(), width, height);
                break;
            case 1:
                temp = new TravelOneThemeTwo((int) mediaItem.getDuration(), width, height);
                break;
            case 2:
                temp = new TravelOneThemeThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                temp = new TravelOneThemeFour((int) mediaItem.getDuration(), width, height);
                break;
            case 4:
                temp = new TravelOneThemeFive((int) mediaItem.getDuration(), width, height);
                break;
        }
