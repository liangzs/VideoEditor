 ...
 * 左y轴翻转进，左下角出z轴翻转出
 */
public class CDSixThemeTwo extends BaseThemeExample {
    private BaseThemeExample widgetOne;
    private BaseThemeExample widgetTwo;
    private CDSixDrawer mCDSixDrawer;

    public CDSixThemeTwo(int totalTime, int width, int height) {
        ...
        mCDSixDrawer = new CDSixDrawer(CDSixSystem.ONE);
    }
    ...
    public void initWidget() {
        ...
        //two
        widgetTwo = new BaseThemeExample(totalTime, 2700, 0, DEFAULT_OUT_TIME, true);
        widgetTwo.setEnterAnimation(new EnterEaseOutElastic(new AnimationBuilder(DEFAULT_ENTER_TIME).setOrientation(AnimateInfo.ORIENTATION.TOP).
                setEnterDelay(1500).setCoordinate(0, -1.1f, 0f, -0.1f).setIsNoZaxis(true).setZView(0)));
        widgetTwo.setStayAction(new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, -0.1f, 0f, -0.1f).setIsNoZaxis(true).setZView(0).build());
        widgetTwo.setOutAnimation(new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, -0.1f, 0f, -0.1f).setFade(1, 0).setIsNoZaxis(true).setZView(0).build());
    }
