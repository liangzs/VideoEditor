 ...
 * 所以这里暂时不回收让系统自己回收
 */
public class WeddingOneThemeManager extends BaseThemeManager {
    private ImageOriginFilter bgImageFilter;
    public static float Z_VIEW = -3f;
    /**
     * 创建缓存对象，离屏buffer
     */
...
//        index = 4;
        switch (index % 5) {
            case 0:
                actionRender = new WeddingActionOne((int) mediaItem.getDuration());
                break;
            case 1:
                actionRender = new WeddingActionTwo((int) mediaItem.getDuration());
                break;
            case 2:
                actionRender = new WeddingActionThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                actionRender = new WeddingActionFour((int) mediaItem.getDuration());
                break;
            case 4:
                actionRender = new WeddingActionFive((int) mediaItem.getDuration(), width, height);
                break;
            default:
                actionRender = new WeddingActionOne((int) mediaItem.getDuration());
                break;
    ...
    public void ratioChange() {
        switch (ConstantMediaSize.ratioType) {
            case _1_1:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/wedding_one_theme_bg11" + ConstantMediaSize.SUFFIX));
                Z_VIEW = -2.5f;
                break;
            case _9_16:
            case _3_4:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/wedding_one_bg" + ConstantMediaSize.SUFFIX));
                break;
            case _4_3:
            case _16_9:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/wedding_one_theme_bg169" + ConstantMediaSize.SUFFIX));
                Z_VIEW = -2f;
                break;
        }
    }
...
//        index = 4;
        switch (index % 5) {
            case 0:
                temp = new WeddingActionOne((int) mediaItem.getDuration());
                break;
            case 1:
                temp = new WeddingActionTwo((int) mediaItem.getDuration());
                break;
            case 2:
                temp = new WeddingActionThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                temp = new WeddingActionFour((int) mediaItem.getDuration());
                break;
            case 4:
                temp = new WeddingActionFive((int) mediaItem.getDuration(), width, height);
                break;
            default:
                temp = new WeddingActionOne((int) mediaItem.getDuration());
        }
