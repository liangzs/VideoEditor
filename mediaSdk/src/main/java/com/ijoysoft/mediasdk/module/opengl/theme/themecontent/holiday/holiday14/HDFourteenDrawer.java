 ...
 * 最后在fragment中，重写计划
 */
public class HDFourteenDrawer implements IParticleRender {
    ...
    private final String fragment = "precision mediump float; \n" +
            ...
            "    gl_FragColor=vec4(v_Color, 1.0) * texture2D(u_TextureUnit, vec2( nx+0.5,ny+0.5));\n" +
            "}\n";

    private float[] viewProjectionMatrix = MatrixUtils.getOriginalMatrix();

    private ParticleShaderProgram particleProgram;
    private HDFourteenSystem particleSystem;
    private long globalStartTime;
    private int texture;

    public HDFourteenDrawer() {
    ...
    public void onSurfaceCreated() {
        ...
        particleSystem = new HDFourteenSystem(10);
        globalStartTime = System.currentTimeMillis();

        texture = OpenGlUtils.loadTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/hd_fourteen_particle" + ConstantMediaSize.SUFFIX));
