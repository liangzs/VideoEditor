package com.ijoysoft.mediasdk.module.opengl.theme.themecontent.love.love1.particle;
 ...
 * @ description
 */
public class LoveOneLoveSystem extends BaseParticleSystem {
    ...
    public LoveOneLoveSystem(int maxParticleCount) {
    ...
    private void addParticleImpl(int index, float particleStartTime) {
        ...
        //记录生命周期时间
        float temp = random.nextInt(3) + 3;
        particles[currentOffset++] = temp;//
        particles[currentOffset++] = temp;//1-4s生命周期

        temp = (4 + random.nextInt(3)) * ConstantMediaSize.getMinWH() / 30;//(20,25,30)
//        Log.i("test", "particle-size:" + temp + ",ConstantMediaSize.showViewWidth / 10:" + ConstantMediaSize.showViewWidth / 30);
        ...
        particles[currentOffset++] = random.nextInt(2);
        vertexArray.updateBuffer(particles, particleOffset, TOTAL_COUNT);
    }
