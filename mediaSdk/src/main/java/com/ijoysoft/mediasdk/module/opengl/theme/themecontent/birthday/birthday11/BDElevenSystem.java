...
public class BDElevenSystem extends BaseParticleSystem {
    ...
    private static final int TOTAL_COUNT =
            LIFETIME_COMPOMENT_COUNT + PARTICLE_POINT_SIZE_COUNT +
    ...
    public BDElevenSystem(int maxParticleCount) {
    ...
    private void addParticleImpl(int index, float particleStartTime, boolean isInit) {
        ...
        temp = (4 + random.nextInt(3)) * ConstantMediaSize.getMinWH() / 20;//(20,25,30)
        if (isInit) {
            particles[currentOffset++] = temp;
        } else {
