 ...
 * 正方形形状
 */
public class ShadeSplitTransitionFilter extends TransitionFilter {
    ...
    private static final String FRAGMENT = FRAGMENT_HEAD +
            "uniform int reverse;\n" +
            "float split(float w) {\n" +
            "  float y0 = w * abs(textureCoordinate.x - 0.5) + 1.0;\n" +
            "  float y1 = w * abs(textureCoordinate.x - 0.5) - 0.5*w;\n" +
            "  float alpha = clamp((y0+progress*(-1.0-0.5*w)), 0.0, 1.0);"+
            "   return alpha;\n" +
            "}\n" +
            "float splitRe(float w) {\n" +
            "  float y0 = w * abs(textureCoordinate.x - 0.5) + 1.0;\n" +
            "  float y1 = w * abs(textureCoordinate.x - 0.5) - 0.5*w;\n" +
            "  float alpha = clamp((y1+progress*(1.0+0.5*w)), 0.0, 1.0);"+
            "   return alpha;\n" +
            "}\n" +
            "void main() {\n" +
            "float w=20.0;" +
            ...
            " if(reverse==0){ " +
            "gl_FragColor = mix(texColor1, texColor2, split(w));" +
            "return;" +
            "} " +
            "gl_FragColor = mix(texColor2,texColor1 , splitRe(w));" +
            "}";


    public ShadeSplitTransitionFilter(TransitionType transitionType) {
