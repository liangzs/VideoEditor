 ...
 * 上进，上下悬停，放大，左划出
 */
public class VTTweleThemeOne extends BDEightteenBaseExample {
    ...
    public VTTweleThemeOne(int totalTime) {
        super(totalTime, DEFAULT_ENTER_TIME, 2000, 1000, DEFAULT_OUT_TIME);
        stayAction = new StayAnimation(2000, AnimateInfo.STAY.SPRING_Y, -2.5f);
        stayNext = new AnimationBuilder(weightTime).setScale(1.0f, 1.2f).setZView(-2.5f).build();
        enterAnimation = new AnimationBuilder(DEFAULT_ENTER_TIME).setCoordinate(0, -2.5f, 0, 0).setZView(-2.5f).build();
        outAnimation = new WaveAnimation(new AnimationBuilder(DEFAULT_OUT_TIME).
                setCoordinate(0, 0f, -2.5f, 0).setZView(-2.5f).setScale(1.2f, 1.2f));
        setZView(-2.5f);
    }
