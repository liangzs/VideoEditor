 ...
 * 上进，上下悬停，左划出
 */
public class VTFourThemeOne extends BaseThemeExample {

    public VTFourThemeOne(int totalTime) {
        ...
        enterAnimation = new AnimationBuilder(DEFAULT_ENTER_TIME).setCoordinate(0, -2.5f, 0, 0).
                setConors(AnimateInfo.ROTATION.X_ANXIS_RE, 0, 0).setZView(VTFourThemeManager.CUSTOM_ZVIEW).build();
        outAnimation = new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, -2.5f, 0).
                setConors(AnimateInfo.ROTATION.Y_ANXIS_RE, 0, 180).setZView(VTFourThemeManager.CUSTOM_ZVIEW).build();
    }
