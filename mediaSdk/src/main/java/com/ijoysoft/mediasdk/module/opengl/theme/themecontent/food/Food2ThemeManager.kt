 ...
 */
class Food2ThemeManager : BaseTimeThemeManager(), IBaseTimeCusFragmeng {
    ...
    override fun intWidget(index: Int, widgetTimeExample: WidgetTimeExample?) {
        widgetTimeExample?.apply {
            when (index) {
                0 -> {
                    adjustScaling(-0.4f, 0.8f, (width > height).ternary(3.5f, 2.2f))
                }
                1 -> {
                    adjustScaling(0.4f, -0.8f, (width > height).ternary(3.5f, 2.2f))
                }
