...
public class WedTenSystemTwo extends BaseParticleSystem {
    ...
    public WedTenSystemTwo(int maxParticleCount) {
    ...
    private void updateParticle(int index) {
        ...
        if (current < 0.3) {//准备消失
            temp = particles[currentOffset + 2] - 10;
            temp = temp < 0 ? 0 : temp;
            particles[currentOffset + 2] = temp;
        }
        if (origin - current < 0.3) {
            current = particles[currentOffset + 2] + 10;
    ...
    private void addParticleImpl(int index, float particleStartTime, boolean isInit) {
        final int particleOffset = index * TOTAL_COUNT;
        int currentOffset = particleOffset;
        Geometry.Point position = new Geometry.Point(random.nextFloat() * -2f + 1f, random.nextFloat() - 1f, 0f);
        //记录生命周期时间
        float temp = random.nextInt(4) + 1;
        particles[currentOffset++] = temp;//
        particles[currentOffset++] = temp;//1-4s生命周期

        temp = (3 + random.nextInt(5)) * ConstantMediaSize.getMinWH() / 40;//(20,25,30)
        if (isInit) {
        ...
        particles[currentOffset++] = random.nextInt(5) * 1f;
        vertexArray.updateBuffer(particles, particleOffset, TOTAL_COUNT);
    }
