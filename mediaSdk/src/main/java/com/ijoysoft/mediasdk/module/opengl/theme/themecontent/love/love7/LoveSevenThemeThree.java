 ...
 * @ description
 */
public class LoveSevenThemeThree extends BaseBlurThemeExample {
    private BaseThemeExample widgetBorder;
    private BaseThemeExample widgetOne;

    public LoveSevenThemeThree(int totalTime, boolean isNoZaxis, int width, int height) {
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float centerX = width == height ? 0.74f : (width < height ? 0.69f : 0.86f);
        float centerY = width == height ? 0.68f : (width < height ? 0.82f : 0.75f);
