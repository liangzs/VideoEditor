 ...
 * @ description
 */
public class TravelTwelveThemeFive extends BaseBlurThemeExample {
    ...
    public TravelTwelveThemeFive(int totalTime, boolean isNoZaxis, int width, int height) {
        ...
        centerY = width == height ? 0f : (width < height ? 0f : 0f);
        ...
        widgetFive.setEnterAnimation(new AnimationBuilder(DEFAULT_ENTER_TIME).setIsNoZaxis(true).setZView(0).setScale(0.01f, 1f).build());
        widgetFive.getEnterAnimation().setIsSubAreaWidget(true, centerX, centerY);
    }
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        scaleX = width == height ? 0 : (width < height ? 10f : 0);
        scaleY = width == height ? 0 : (width < height ? 10f : 0);
        centerX = width == height ? 0.82f : (width < height ? 0.82f : 0.82f);
        centerY = width == height ? 0.6f : (width < height ? 0.4f : 0.6f);
        widgetFive.init(mimaps.get(5), width, height);
        widgetFive.adjustScaling(width, height,
                mimaps.get(5).getWidth(), mimaps.get(5).getHeight(), centerX, centerY, scaleX, scaleY);
    }
