 ...
 * * x^3
 */
public class EaseInCubic implements IMoveAction {
     ...
     * @param rotate
     */
    public EaseInCubic(boolean transition, boolean scale, boolean rotate) {
        this.transition = transition;
        this.scale = scale;
        this.rotate = rotate;
    }

    public EaseInCubic(boolean transition, boolean scale, boolean rotate, boolean skew) {
        this.transition = transition;
        this.scale = scale;
        this.rotate = rotate;
        this.skew = skew;
    ...
    public float[] action(float duration, float start, float end) {
        ...
        for (int i = 0; i < framesNum; i++) {
            ...
            frameValue = x == 0 ? 0 : x > 1 ? 1 : Math.pow(x, 3);
