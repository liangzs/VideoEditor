...
class MoveLeftAction: BaseThemeTemplate {
    ...
    override fun initEnterAnimation() = AnimationBuilder(totalTime).setStartX(0.5f).setEndX(-0.5f).build()


    override fun initStayAction() = AnimationBuilder(40).setStartX(-0.5f).setEndX(-0.5f).build()

    override fun initOutAnimation() = AnimationBuilder(40).setStartX(-0.5f).setEndX(-0.5f).build()
    ...
    override fun initGif(
        ...
        gifDrawer1?.adjustScaling(width, height, AnimateInfo.ORIENTATION.LEFT_TOP, 0f, if (width == height) 2f else 1.5f)
        gifDrawer2?.adjustScaling(width, height, AnimateInfo.ORIENTATION.BOTTOM, 0f, 2f)
    }
