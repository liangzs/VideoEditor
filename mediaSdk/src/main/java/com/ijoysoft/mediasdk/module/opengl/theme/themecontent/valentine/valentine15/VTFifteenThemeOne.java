 ...
 * 上进，上下悬停，放大，左划出
 */
public class VTFifteenThemeOne extends VTFifteenBaseExample {
    private BaseThemeExample widgetOne;
    private VTFifteenDrawer mVTFifteenDrawer;

    public VTFifteenThemeOne(int totalTime, int width, int height) {
        ...
        stayNext = new AnimationBuilder(weightTime).setScale(1.0f, 1.2f).setZView(zview).build();
        enterAnimation = new AnimationBuilder(DEFAULT_ENTER_TIME).setCoordinate(0, -2.5f, 0, 0).setZView(zview).build();
        outAnimation = new WaveAnimation(new AnimationBuilder(DEFAULT_OUT_TIME).
                setCoordinate(0, 0f, -2.5f, 0).setZView(zview).setScale(1.2f, 1.2f));
