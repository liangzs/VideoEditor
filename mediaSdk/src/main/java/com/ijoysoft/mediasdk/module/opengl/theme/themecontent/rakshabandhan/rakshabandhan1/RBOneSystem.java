...
public class RBOneSystem extends BaseParticleSystem {
    ...
    public RBOneSystem(int maxParticleCount) {
    ...
    public void addParticle(float particleStartTime) {
        if (currentParticleCount < maxParticleCount) {
            //不一定要每次都添加，增加随机性
            if (addParticleImpl(currentParticleCount, particleStartTime)); currentParticleCount++;
            return;
        }
    ...
    private void updateParticle(int index) {
        ...
        if (current < 1) {//准备消失
            temp = particles[currentOffset + 2] - 6;
            temp = temp < 0 ? 0 : temp;
...
//            current = current > origin ? origin : current;
            particles[currentOffset + 2] += 4;
        }
        particles[currentOffset+5] = 0.24f * time * time - 1f;
     ...
     */
    private boolean addParticleImpl(int index, float particleStartTime) {
        if (random.nextInt(100) < 2) {
            final int particleOffset = index * TOTAL_COUNT;
            int currentOffset = particleOffset;
//        Geometry.Point position = new Geometry.Point(random.nextFloat() * -2f + 1f, 0f, 0f);
            //记录生命周期时间
            particles[currentOffset] = 3;
            particles[currentOffset+1] = 3;

            currentOffset+=2;

            int temp = (1 + random.nextInt(3)) * ConstantMediaSize.showViewWidth * 3;//(20,25,30)
            particles[currentOffset++] = 0;//当前大小
            particles[currentOffset++] = temp;//大小

            particles[currentOffset++] = random.nextFloat() * -2f + 1f;
