 ...
 * 所以这里暂时不回收让系统自己回收
 */
public class HDSevenThemeManager extends BaseThemeManager {

    private int width, height;
    //粒子系统

    //由于bitmap的处理比较延迟，所以在对象初始化的时候把这些数据准备好，以防用到时卡动画
    public HDSevenThemeManager() {
        super();
    }
    @Override
...
//        index = 0;
        switch (index % 5) {
            case 0:
                actionRender = new HDSevenThemeOne((int) mediaItem.getDuration());
                break;
            case 1:
                actionRender = new HDSevenThemeTwo((int) mediaItem.getDuration(), width, height);
                break;
            case 2:
                actionRender = new HDSevenThemeThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                actionRender = new HDSevenThemeFour((int) mediaItem.getDuration(), width, height);
                break;
            case 4:
                actionRender = new HDSevenThemeFive((int) mediaItem.getDuration(), width, height);
                break;
        }
...
//        index = 0;
        switch (index % 5) {
            case 0:
                temp = new HDSevenThemeOne((int) mediaItem.getDuration());
                break;
            case 1:
                temp = new HDSevenThemeTwo((int) mediaItem.getDuration(), width, height);
                break;
            case 2:
                temp = new HDSevenThemeThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                temp = new HDSevenThemeFour((int) mediaItem.getDuration(), width, height);
                break;
            case 4:
                temp = new HDSevenThemeFive((int) mediaItem.getDuration(), width, height);
                break;
        }
