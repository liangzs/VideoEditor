...
public class TextureMovieEncoder implements Runnable {
    ...
    public TextureMovieEncoder(VideoEncoderCore encoderCore) {
        LogUtils.d(TAG, "Encoder: startRecording()");

        mVideoEncoder = encoderCore;

        synchronized (mReadyFence) {
            if (mRunning) {
                LogUtils.w(TAG, "Encoder thread already running");
                return;
            }
            mRunning = true;
    ...
    public void run() {
        ...
        LogUtils.d(TAG, "Encoder thread exiting");
        synchronized (mReadyFence) {
    ...
    private static class EncoderHandler extends Handler {
        ...
        public void handleMessage(Message inputMessage) {
            ...
            if (encoder == null) {
                LogUtils.w(TAG, "EncoderHandler.handleMessage: encoder is null");
                return;
            }
    ...
    private void handleStopRecording() {
        LogUtils.d(TAG, "handleStopRecording");
