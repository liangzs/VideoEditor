...
public class VTSixSystemTwo extends BaseParticleSystem {
    ...
    private static final int PARTICLE_POINT_CORNOR_COUNT = 1;
    private static final int TOTAL_COUNT =
            LIFETIME_COMPOMENT_COUNT +
                    POSITION_COMPONENT_COUNT
                    + VECTOR_COMPONENT_COUNT
                    + PARTICLE_START_TIME_COMPONENT_COUNT + PARTICLE_POINT_SIZE_COUNT + PARTICLE_POINT_CORNOR_COUNT;

    protected static int STRIDE = TOTAL_COUNT * BYTES_PER_FLOAT;

    public VTSixSystemTwo(int maxParticleCount) {
    ...
    public void bindData(ParticleShaderProgram particleProgram) {
        ...
        vertexArray.setVertexAttribPointer(dataOffset,
                ...
                PARTICLE_POINT_CORNOR_COUNT, STRIDE);
    ...
    private void addParticleImpl(int index, float particleStartTime) {
        ...
        //记录生命周期时间
        float temp = random.nextInt(3) + 4;
        particles[currentOffset++] = temp;//1-4s生命周期

        temp = (2 + random.nextInt(2)) * ConstantMediaSize.getMinWH() / 40;
        ...
        particles[currentOffset++] = random.nextInt(36) * 10;
        vertexArray.updateBuffer(particles, particleOffset, TOTAL_COUNT);
    }
