 ...
 * 所以这里暂时不回收让系统自己回收
 */
public class HoliThreeThemeManager extends BaseThemeManager {
    ...
    public void drawPrepare(MediaItem mediaItem, int index) {
        ...
        switch (index % 4) {
            case 0:
                actionRender = new HoliThreeThemeOne((int) mediaItem.getDuration(), true, width, height);
                break;
            case 1:
                actionRender = new HoliThreeThemeTwo((int) mediaItem.getDuration(), true);
                break;
            case 2:
                actionRender = new HoliThreeThemeThree((int) mediaItem.getDuration(), true);
                break;
            case 3:
                actionRender = new HoliThreeThemeFour((int) mediaItem.getDuration(), true, width, height);
                break;
        }
...
//        bgImageFilter.draw();
    ...
    public IAction getNextAction(MediaItem mediaItem, int index) {
        ...
        switch (index % 4) {
            case 0:
                temp = new HoliThreeThemeOne((int) mediaItem.getDuration(), true, width, height);
                break;
            case 1:
                temp = new HoliThreeThemeTwo((int) mediaItem.getDuration(), true);
                break;
            case 2:
                temp = new HoliThreeThemeThree((int) mediaItem.getDuration(), true);
                break;
            case 3:
                temp = new HoliThreeThemeFour((int) mediaItem.getDuration(), true, width, height);
                break;
        }
