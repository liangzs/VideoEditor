 ...
 * one和two承载前两文件展示移动
 */
public class WeddingFourThemeThree extends BaseThemeExample {
    private BaseThemeExample widgetOne;
    private BaseThemeExample widgetTwo;
    private int width, height;

    public WeddingFourThemeThree(int totalTime, int width, int height) {
        super(totalTime, DEFAULT_ENTER_TIME, 0, DEFAULT_OUT_TIME);
        enterAnimation = new AnimationBuilder(DEFAULT_ENTER_TIME).setZView(WeddingFourThemeManager.Z_VIEW).setCoordinate(WeddingFourThemeManager.Z_VIEW, -WeddingFourThemeManager.Z_VIEW, 0, 0).
                setConors(AnimateInfo.ROTATION.Z_ANXIS, 0, 360).setWidthHeight(width, height).build();
        outAnimation = new AnimationBuilder(DEFAULT_OUT_TIME).setZView(WeddingFourThemeManager.Z_VIEW).setCoordinate(0, 0f, 1.5f * WeddingFourThemeManager.Z_VIEW, 0f).build();
    ...
    public void setPreTeture(List<Integer> listTexture, List<Integer> listConors, List<float[]> listPos, List<GPUImageFilter> listAfilter) {
        ...
        widgetOne.setOutAnimation(new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, 1.5f * WeddingFourThemeManager.Z_VIEW, 0f)
        ...
        widgetTwo.setEnterAnimation(new AnimationBuilder(3400).setWidthHeight(width, height).
                setConors(AnimateInfo.ROTATION.Z_ANXIS, conor, conor).setZView(WeddingFourThemeManager.Z_VIEW).build());
        widgetTwo.setOutAnimation(new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, 1.5f * WeddingFourThemeManager.Z_VIEW, 0f)
                .setWidthHeight(width, height).setConors(AnimateInfo.ROTATION.Z_ANXIS, conor, conor).setZView(WeddingFourThemeManager.Z_VIEW).build());
