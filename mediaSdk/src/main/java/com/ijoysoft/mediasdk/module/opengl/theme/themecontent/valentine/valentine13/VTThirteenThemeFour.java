 ...
 * 左上角z轴翻滚进，右下角z轴滚出
 */
public class VTThirteenThemeFour extends BaseThemeExample {
    ...
    public VTThirteenThemeFour(int totalTime, int width, int height) {
    ...
    public void initWidget() {
        ...
        widgetFour.setEnterAnimation(new AnimationBuilder(DEFAULT_ENTER_TIME).setZView(-2.5f).setCoordinate(0, -2f, 0f, 0f)
                .build());
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float scaleY = width < height ? 12 : 6;
        float centerX = width < height ? -0.5f : -0.9f;
        float centerY = width < height ? 0.9f : -0.5f;
        widgetOne.init(mimaps.get(0), width, height);
        originOne = widgetOne.adjustScaling(width / devideX, height / devideY, mimaps.get(0).getWidth(),
                mimaps.get(0).getHeight(), centerX, centerY, scaleX, scaleY);
        //two
        centerX = width < height ? 0.0f : -0.9f;
        centerY = width < height ? 0.9f : -0.0f;
        widgetTwo.init(mimaps.get(0), width, height);
        originTwo = widgetTwo.adjustScaling(width / devideX, height / devideY, mimaps.get(0).getWidth(),
                mimaps.get(0).getHeight(), centerX, centerY, scaleX, scaleY);
        centerX = width < height ? 0.5f : -0.9f;
        centerY = width < height ? 0.9f : 0.5f;
        widgetThree.init(mimaps.get(0), width, height);
        originThree = widgetThree.adjustScaling(width / devideX, height / devideY, mimaps.get(0).getWidth(),
                mimaps.get(0).getHeight(), centerX, centerY, scaleX, scaleY);
        ...
        widgetFour.adjustScaling(width / devideX, height / devideY, mimaps.get(0).getWidth(), mimaps.get(0).getHeight(),
