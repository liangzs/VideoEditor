 ...
 * @ description
 */
public class LoveEightThemeThree extends BaseBlurThemeExample {
    private BaseThemeExample widgetBorder;
    private BaseThemeExample widgetOne;

    public LoveEightThemeThree(int totalTime, boolean isNoZaxis, int width, int height) {
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float scaleY = width == height ? 4 : (width < height ? 4 : 4);
        float centerX = width == height ? 0.82f : (width < height ? 0.75f : 0.9f);
        float centerY = width == height ? 0.72f : (width < height ? 0.75f : 0.73f);
