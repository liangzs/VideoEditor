 ...
 * stay 放大-缩小-放大
 */
public class BDFifteenThemeTwo extends BaseBlurThemeExample {
    ...
    public BDFifteenThemeTwo(int totalTime, boolean isNoZaxis) {
...
//        widgetOne.setOutAnimation(new AnimationBuilder(DEFAULT_ENTER_TIME).setZView(0).build());
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float centerX = width < height ? 0.1f : (width == height ? 0.7f : 0.7f);
        float centerY = width < height ? -0.8f : (width == height ? -0.8f : -0.8f);
        widgetTwo.init(mimaps.get(3), width, height);
