 ...
 * 条形块分阶移动
 */
public class ShadeBarCrossYAxisTransitionFilter extends TransitionFilter {
    ...
    private static final String FRAGMENT = FRAGMENT_HEAD +
            ...
            "    float flag = step(0.0, fract(textureCoordinate.x * w) * 2.0 - 1.0);\n" +
            "    texCoord1.y -= progress;\n" +
            "    texCoord2.y += progress;\n" +
            "    vec4 texColor1 = texture2D(vTexture1, texCoord1);\n" +
            "    texColor1 *= flag * step(progress, textureCoordinate.y);\n" +
            "    vec4 texColor2 = texture2D(vTexture1, texCoord2);\n" +
            "    texColor2 *= (1.0 - flag) * step(progress, 1.0 - textureCoordinate.y);\n" +
    ...
    public ShadeBarCrossYAxisTransitionFilter(TransitionType transitionType) {
