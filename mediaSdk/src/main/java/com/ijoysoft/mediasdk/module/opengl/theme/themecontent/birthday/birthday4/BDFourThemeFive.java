 ...
 * 左下角z轴翻滚进，右y轴翻转出
 */
public class BDFourThemeFive extends BaseThemeExample {

    private BaseThemeExample widgetOne;

    public BDFourThemeFive(int totalTime, int width, int height) {
    ...
    public void initWidget() {
        ...
        widgetOne.setEnterAnimation(new AnimationBuilder(DEFAULT_ENTER_TIME).setZView(-2.5f).setCoordinate(0f, 2f, 0, 0).
                setConors(AnimateInfo.ROTATION.NONE, 0, 0).build());
        widgetOne.setOutAnimation(new AnimationBuilder(DEFAULT_ENTER_TIME).setZView(-2.5f).setCoordinate(0f, 0f, 2f, 0).
                setConors(AnimateInfo.ROTATION.NONE, 0, 0).build());
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float scaleX = width < height ? 2.5f : (width == height ? 2.5f : 3);
        float centerX = 0;
        float centerY = width < height ? 0.65f : (width == height ? 0.6f : 0.65f);
