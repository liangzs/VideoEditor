...
import static com.ijoysoft.mediasdk.module.opengl.theme.themecontent.love.love16.LoveSixteenThemeManager.LOVESIXTEEN_ZVIEW;

/**
 * 左下角z轴翻滚进，右y轴翻转出
 */
public class LoveSixteenThemeOne extends BaseThemeExample {

    private BaseThemeExample widgetOne;

    public LoveSixteenThemeOne(int totalTime, int width, int height) {
        super(totalTime, DEFAULT_ENTER_TIME, DEFAULT_STAY_TIME, DEFAULT_OUT_TIME);
        stayAction = new StayAnimation(DEFAULT_STAY_TIME, AnimateInfo.STAY.ROTATE_Z, width, height, LOVESIXTEEN_ZVIEW);
        enterAnimation = new AnimationBuilder(DEFAULT_ENTER_TIME).setCoordinate(-2f, 2f, 0, 0).
                setConors(AnimateInfo.ROTATION.Z_ANXIS, 0, 360).setWidthHeight(width, height).setZView(LOVESIXTEEN_ZVIEW).build();
        outAnimation = new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, 2f, 0f).
                setConors(AnimateInfo.ROTATION.Y_ANXIS_RE, 0, 90).setZView(LOVESIXTEEN_ZVIEW).build();
        setZView(LOVESIXTEEN_ZVIEW);
    }
