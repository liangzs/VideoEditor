 ...
 * 2立方曲线，由慢到快，适合出场
 * * x^2
 * 比{@link EaseInCubic}速度先对缓和
 */
public class EaseInPow implements IMoveAction {
     ...
     * @param rotate
     */
    public EaseInPow(boolean transition, boolean scale, boolean rotate) {
        this.transition = transition;
        this.scale = scale;
        this.rotate = rotate;
    }

    public EaseInPow(boolean transition, boolean scale, boolean rotate, boolean skew) {
        this.transition = transition;
        this.scale = scale;
        this.rotate = rotate;
        this.skew = skew;
    ...
    public float[] action(float duration, float start, float end) {
        ...
        for (int i = 0; i < framesNum; i++) {
            ...
            frameValue = x == 0 ? 0 : x > 1 ? 1 : Math.pow(x, 2);
