 ...
 * 最后在fragment中，重写计划
 */
public class BDSixteenDrawerOne implements IParticleRender {
    ...
    private final String fragment = "precision mediump float; \n" +
            ...
            "        gl_FragColor=vec4(v_Color, 1.0) * texture2D(u_Texture5, vec2( nx+0.5,ny+0.5));\n" +
            "    }else if(v_Paticle==4.0){\n" +
            "        gl_FragColor=vec4(v_Color, 1.0) * texture2D(u_Texture6, vec2( nx+0.5,ny+0.5));\n" +
            "    }\n" +
    ...
    private BDSixteenSystemOne particleSystem;
    ...
    public void onSurfaceCreated() {
        ...
        particleSystem = new BDSixteenSystemOne(10);
        globalStartTime = System.currentTimeMillis();

        textures[0] = OpenGlUtils.loadTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/bd_sixteen_particle1" + ConstantMediaSize.SUFFIX));
        textures[1] = OpenGlUtils.loadTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/bd_sixteen_particle2" + ConstantMediaSize.SUFFIX));
        textures[2] = OpenGlUtils.loadTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/bd_sixteen_particle3" + ConstantMediaSize.SUFFIX));
        textures[3] = OpenGlUtils.loadTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/bd_sixteen_particle4" + ConstantMediaSize.SUFFIX));
        textures[4] = OpenGlUtils.loadTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/bd_sixteen_particle5" + ConstantMediaSize.SUFFIX));
