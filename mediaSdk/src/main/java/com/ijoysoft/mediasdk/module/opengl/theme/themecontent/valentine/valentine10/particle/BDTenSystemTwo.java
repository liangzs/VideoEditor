 ...
 * 锥子型，不停放大
 */
public class BDTenSystemTwo extends BaseParticleSystem {
    ...
    private static final int starNum = 8;
    private float x;
    private float y;

    public BDTenSystemTwo() {
    ...
    private void addParticleImpl(int index, float time, boolean init) {
        final int particleOffset = index * TOTAL_COUNT;
        Geometry.Point point = new Geometry.Point(0f, 0.7f, 0);
        int currentOffset = particleOffset;
        //记录生命周期时间
    ...
    private int evaluatePointSize() {
        ...
        int b;
        if (a < 60) {
            b = Math.min(ConstantMediaSize.showViewWidth, ConstantMediaSize.showViewHeight) / 10;
        } else if (a < 80) {
            b = Math.min(ConstantMediaSize.showViewWidth, ConstantMediaSize.showViewHeight) / 15;
        } else {
            b = Math.min(ConstantMediaSize.showViewWidth, ConstantMediaSize.showViewHeight) / 18;
        }
        return b;
    }
