 ...
 * 欧南节3
 */
public class Onam3ThemeManager extends BaseThemeManager {
    private ImageOriginFilter bgImageFilter;
    private int width, height;
    public static float BD_ONE_ZVIEW = 0;

    //由于bitmap的处理比较延迟，所以在对象初始化的时候把这些数据准备好，以防用到时卡动画
    public Onam3ThemeManager() {
        super();
...
//        index=0;
        switch (index % 6) {
            case 0:
                actionRender = new Onam3Action0((int) mediaItem.getDuration(), width, height);
                break;
            case 1:
                actionRender = new Onam3Action1((int) mediaItem.getDuration(), width, height);
                break;
            case 2:
                actionRender = new Onam3Action2((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                actionRender = new Onam3Action3((int) mediaItem.getDuration(), width, height);
                break;
            case 4:
                actionRender = new Onam3Action4((int) mediaItem.getDuration(), width, height);
                break;
            case 5:
                actionRender = new Onam3Action5((int) mediaItem.getDuration(), width, height);
                break;
        }
...
//        bgImageFilter.setBackgroundTexture(BitmapFactory.decodeResource(MediaSdk.getInstance().getResouce(), R.mipmap.rb_bg));
        bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/onam3_bg" + ConstantMediaSize.SUFFIX));
...
//        index = 0;
        switch (index % 6) {
            case 1:
                temp = new Onam3Action1((int) mediaItem.getDuration(), width, height);
                break;
            case 2:
                temp = new Onam3Action2((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                temp = new Onam3Action3((int) mediaItem.getDuration(), width, height);
                break;
            case 4:
                temp = new Onam3Action4((int) mediaItem.getDuration(), width, height);
                break;
            case 5:
                temp = new Onam3Action5((int) mediaItem.getDuration(), width, height);
                break;
            default:
                temp = new Onam3Action0((int) mediaItem.getDuration(), width, height);
        }
