...
public class GreetFiveSystem extends BaseParticleSystem {
    ...
    public GreetFiveSystem(int maxParticleCount) {
    ...
    private void addParticleImpl(int index, float particleStartTime, boolean isInit) {
        final int particleOffset = index * TOTAL_COUNT;
        int currentOffset = particleOffset;
        Geometry.Point position = new Geometry.Point(random.nextFloat() * -2f + 1f, random.nextFloat() - 1.5f, 0f);
        //记录生命周期时间
        float temp = random.nextInt(4) + 1;
        particles[currentOffset++] = temp;//
        particles[currentOffset++] = temp;//1-4s生命周期

        temp = (4 + random.nextInt(3)) * 10f;//(20,25,30)
        if (isInit) {
            particles[currentOffset++] = temp;
        } else {
        ...
        particles[currentOffset++] = (random.nextFloat()) * 0.4f;
        particles[currentOffset++] = 0f;

        particles[currentOffset++] = particleStartTime;
        particles[currentOffset++] = evaluateFlowerType();
        vertexArray.updateBuffer(particles, particleOffset, TOTAL_COUNT);
    }
