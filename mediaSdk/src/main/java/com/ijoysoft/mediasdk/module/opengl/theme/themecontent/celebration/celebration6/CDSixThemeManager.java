 ...
 * 所以这里暂时不回收让系统自己回收
 */
public class CDSixThemeManager extends BaseThemeManager {
    /**
     * 创建缓存对象，离屏buffer
     */
...
//        index = 2;
        switch (index % 5) {
            case 0:
                actionRender = new CDSixThemeOne((int) mediaItem.getDuration());
                break;
            case 1:
                actionRender = new CDSixThemeTwo((int) mediaItem.getDuration(), width, height);
                break;
            case 2:
                actionRender = new CDSixThemeThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                actionRender = new CDSixThemeFour((int) mediaItem.getDuration(), width, height);
                break;
            case 4:
    ...
    public void ratioChange() {
        switch (ConstantMediaSize.ratioType) {
            case _9_16:
            case _3_4:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/cd_six_theme_bg916" + ConstantMediaSize.SUFFIX));
                break;
            case _1_1:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/cd_six_theme_bg11" + ConstantMediaSize.SUFFIX));
                break;
            case _16_9:
            case _4_3:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/cd_six_theme_bg169" + ConstantMediaSize.SUFFIX));
                break;
        }
    }
...
//        index = 2;
        switch (index % 5) {
            case 0:
                temp = new CDSixThemeOne((int) mediaItem.getDuration());
                break;
            case 1:
                temp = new CDSixThemeTwo((int) mediaItem.getDuration(), width, height);
                break;
            case 2:
                temp = new CDSixThemeThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                temp = new CDSixThemeFour((int) mediaItem.getDuration(), width, height);
                break;
            case 4:
