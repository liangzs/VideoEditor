 ...
 * 左上角z轴翻滚进，右下角z轴滚出
 */
public class VTSevenThemeFour extends BaseThemeExample {
    ...
    public VTSevenThemeFour(int totalTime, int width, int height) {
        ...
        outAnimation = new AnimationBuilder(DEFAULT_OUT_TIME).setCoordinate(0, 0f, 2f, 2f).
                setConors(AnimateInfo.ROTATION.Z_ANXIS, 0, 180).setWidthHeight(width, height).setZView(-2.5f).build();
    }
    ...
    public void init(Bitmap bitmap, Bitmap tempBit, List<Bitmap> mimaps, int width, int height) {
        ...
        float scale = width <= height ? 6f : 5f;
        float centerX = -0.5f;
        float centerY = 0.8f;
