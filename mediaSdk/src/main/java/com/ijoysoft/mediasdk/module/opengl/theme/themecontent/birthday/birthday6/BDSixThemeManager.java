 ...
 * 所以这里暂时不回收让系统自己回收
 */
public class BDSixThemeManager extends BaseThemeManager {
...
//        index = 4;
        switch (index % 5) {
            case 0:
                actionRender = new BDSixThemeOne((int) mediaItem.getDuration());
                break;
            case 1:
                actionRender = new BDSixThemeTwo((int) mediaItem.getDuration());
                break;
            case 2:
                actionRender = new BDSixThemeThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                actionRender = new BDSixThemeFour((int) mediaItem.getDuration());
                break;
            case 4:
                actionRender = new BDSixThemeFive((int) mediaItem.getDuration());
                break;
        }
    ...
    public void ratioChange() {
        switch (ConstantMediaSize.ratioType) {
            case _16_9:
            case _4_3:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/bd_six_bg169" + ConstantMediaSize.SUFFIX));
                break;
            case _3_4:
            case _9_16:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/bd_six_theme_bg" + ConstantMediaSize.SUFFIX));
                break;
            case _1_1:
                bgImageFilter.setBackgroundTexture(BitmapUtil.decriptImage(ConstantMediaSize.themePath + "/bd_six_bg169" + ConstantMediaSize.SUFFIX));
                break;
...
//        index = 4;
        switch (index % 5) {
            case 0:
                temp = new BDSixThemeOne((int) mediaItem.getDuration());
                break;
            case 1:
                temp = new BDSixThemeTwo((int) mediaItem.getDuration());
                break;
            case 2:
                temp = new BDSixThemeThree((int) mediaItem.getDuration(), width, height);
                break;
            case 3:
                temp = new BDSixThemeFour((int) mediaItem.getDuration());
                break;
            case 4:
                temp = new BDSixThemeFive((int) mediaItem.getDuration());
                break;
        }
