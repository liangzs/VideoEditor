...
package com.ijoysoft.mediasdk.view.teture.gles;
...
public class GLThread extends Thread {
    ...
    public void run() {
        ...
        LogUtils.i(TAG, "starting tid=" + getId());
                ...
                synchronized (sGLThreadManager) {
                    ...
                    while (true) {
                        ...
                        if (mPaused != mRequestPaused) {
                            ...
                            LogUtils.i(TAG, "mPaused is now " + mPaused + " tid=" + getId());
                        }

                        // Have we lost the EGL context?
                        if (lostEglContext) {
                            LogUtils.i(TAG, "lostEglContext");
                        ...
                        if (pausing && mHaveEglSurface) {
                            LogUtils.i(TAG, "releasing EGL surface because paused tid=" + getId());
                            stopEglSurfaceLocked();
                        }

                        // Have we lost the SurfaceView surface?
                        if ((!mHasSurface) && (!mWaitingForSurface)) {
                            LogUtils.i(TAG, "noticed surfaceView surface lost tid=" + getId());
                            if (mHaveEglSurface) {
                                stopEglSurfaceLocked();
                            }
                        ...
                        if (mHasSurface && mWaitingForSurface) {
                            LogUtils.i(TAG, "noticed surfaceView surface acquired tid=" + getId());
...
//                            Log.i(TAG, "sending render notification tid=" + getId());
                        ...
                        if (readyToDraw()) {
                            ...
                            if (mHaveEglSurface) {
                                if (mSizeChanged) {
                                    ...
                                    LogUtils.i(TAG, "noticing that we want render notification tid=" + getId());
                        ...
                        if (LOG_THREADS) {
                            LogUtils.v(TAG, "waiting tid=" + getId()
                ...
                if (createEglSurface) {
                    LogUtils.w(TAG, "egl createSurface");
                ...
                if (createEglContext) {
                    LogUtils.w("GLThread", "onSurfaceCreated");
                ...
                if (sizeChanged) {
                    LogUtils.w(TAG, "onSurfaceChanged(" + w + ", " + h + ")");
                ...
                if (mChoreographerRenderWrapper.canSwap()) {
                    ...
                    switch (swapError) {
                        ...
                        case EGL11.EGL_CONTEXT_LOST:
                            LogUtils.i(TAG, "egl context lost tid=" + getId());
                            lostEglContext = true;
                            break;
                        default:
    ...
    public void surfaceCreated() {
        synchronized (sGLThreadManager) {
            LogUtils.i(TAG, "surfaceCreated tid=" + getId());
            mHasSurface = true;
    ...
    public void surfaceDestroyed() {
        synchronized (sGLThreadManager) {
            LogUtils.i(TAG, "surfaceDestroyed tid=" + getId());
            mHasSurface = false;
            sGLThreadManager.notifyAll();
    ...
    public void onPause() {
        synchronized (sGLThreadManager) {
            LogUtils.i(TAG, "onPause tid=" + getId());
            mRequestPaused = true;
            sGLThreadManager.notifyAll();
            while ((!mExited) && (!mPaused)) {
                LogUtils.i(TAG, "onPause waiting for mPaused.");
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
                    Thread.currentThread().interrupt();
    ...
    public void onResume() {
        synchronized (sGLThreadManager) {
            LogUtils.i(TAG, "onResume tid=" + getId());
            mRequestPaused = false;
            mRequestRender = true;
            mRenderComplete = false;
            sGLThreadManager.notifyAll();
            while ((!mExited) && mPaused && (!mRenderComplete)) {
                LogUtils.i(TAG, "onResume waiting for !mPaused.");
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
                    Thread.currentThread().interrupt();
    ...
    public void onWindowResize(int w, int h) {
        synchronized (sGLThreadManager) {
            LogUtils.d(TAG, "width:" + w + " height:" + h);
            mWidth = w;
            mHeight = h;
            mSizeChanged = true;
            mRequestRender = true;
            ...
            while (!mExited && !mPaused && !mRenderComplete
                    && ableToDraw()) {
                LogUtils.i(TAG, "onWindowResize waiting for render complete from tid=" + getId());
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
                    Thread.currentThread().interrupt();
    ...
    private static class GLThreadManager {
        ...
        public synchronized void threadExiting(GLThread thread) {
            LogUtils.i(TAG, "exiting tid=" + thread.getId());
            thread.mExited = true;
            if (mEglOwner == thread) {
                mEglOwner = null;
            }
    ...
    public static class DefaultContextFactory implements EGLContextFactory {
        ...
        public void destroyContext(EGL10 egl, EGLDisplay display,
            ...
            if (!egl.eglDestroyContext(display, context)) {
                LogUtils.e(TAG, "DefaultContextFactory " + "display:" + display + " context: " + context);
