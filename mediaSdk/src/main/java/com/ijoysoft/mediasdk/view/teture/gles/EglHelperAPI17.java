...
import static com.ijoysoft.mediasdk.view.teture.gles.EglHelper.formatEglError;
...
public class EglHelperAPI17 implements IEglHelper {
    ...
    public EglContextWrapper start(EglContextWrapper eglContext) {
        LogUtils.w(TAG, "start() tid=" + Thread.currentThread().getId());
        /*
         * Get an EGL instance
         */
        ...
        if (mEglContext == null || mEglContext == EGL14.EGL_NO_CONTEXT) {
            LogUtils.d(TAG, "mEglContext:" + mEglContext);
            mEglContext = null;
            throwEglException("; createContext");
        }
        LogUtils.w(TAG, "createContext " + mEglContext + " tid=" + Thread.currentThread().getId());
    ...
    public boolean createSurface(Object surface) {
        LogUtils.w(TAG, "createSurface()  tid=" + Thread.currentThread().getId());
    ...
    public int swap() {
        if (!EGL14.eglSwapBuffers(mEglDisplay, mEglSurface)) {
            LogUtils.w(TAG, String.format("swap: start get error"));
    ...
    public void destroySurface() {
        LogUtils.w(TAG, "destroySurface()  tid=" + Thread.currentThread().getId());
        destroySurfaceImp();
    }
    ...
    @Override
    public void finish() {
        LogUtils.w(TAG, "finish() tid=" + Thread.currentThread().getId());
        if (mEglContext != null) {
    ...
    public static void logEglErrorAsWarning(String tag, String function, int error) {
        LogUtils.w(tag, formatEglError(function, error));
    }
    ...
    public static void throwEglException(String function, int error) {
        ...
        LogUtils.e(TAG, "throwEglException tid=" + Thread.currentThread().getId() + " " + message);
